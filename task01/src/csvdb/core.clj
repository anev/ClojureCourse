...
(defn data-record [tbl-keys tbl-record]
   (def q (apply key-value-pairs [tbl-keys tbl-record]))
   (apply hash-map q)
  )
...
(defn data-table [tbl]
  (defn table-loop [a-list]
    (when a-list
      (cons (data-record (table-keys tbl) (first a-list))
        (table-loop (next a-list)))))
  (table-loop (next tbl))
)
...
(defn join* [data1 column1 data2 column2]
  ...
  (for [row1 data1
        row2 data2
        :when (= (column1 row1) (column2 row2))
       ]
       (merge row2 row1)
  ))
